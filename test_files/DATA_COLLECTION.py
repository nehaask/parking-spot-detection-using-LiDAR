...
def main():
    ...
    geo_ref = world.get_map().get_georeference()
    print(geo_ref)
    ...
    try:
        while current_frame <= end:
            ...
            for p in points_t:
                ...
                geo_loc = world.get_map().transform_to_geolocation(loc)
                lat_lon_alt_list.append([geo_loc.latitude, geo_loc.longitude, geo_loc.altitude])

            lat_lon_alt_array = np.array(lat_lon_alt_list)

            pcd = o3d.t.geometry.PointCloud()
            pcd.point["latitude"] = o3d.core.Tensor(lat_lon_alt_array[:, 0].reshape((-1, 1)), o3d.core.float64)
            pcd.point["longitude"] = o3d.core.Tensor(lat_lon_alt_array[:, 1].reshape((-1, 1)), o3d.core.float64)
            pcd.point["altitude"] = o3d.core.Tensor(lat_lon_alt_array[:, 2].reshape((-1, 1)), o3d.core.float64)
