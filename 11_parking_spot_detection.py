...
def imu_callback(world, imu):
    snapshot = world.get_snapshot()
    timestamp_seconds = snapshot.timestamp.elapsed_seconds
    imu_car_sensorQueue.put([imu.frame,timestamp_seconds, imu.accelerometer.x, imu.accelerometer.y, imu.accelerometer.z,imu.gyroscope.x,imu.gyroscope.y,imu.gyroscope.z, imu.compass])


def generate_imu_bp(arg, blueprint_library):
    """Generates a CARLA blueprint based on the script parameters"""
    imu_bp = blueprint_library.find('sensor.other.imu')
    if args.imu_noise:
        imu_bp.set_attribute('noise_accel_stddev_x', args.imu_noise_std)
        imu_bp.set_attribute('noise_accel_stddev_y', args.imu_noise_std)
        imu_bp.set_attribute('noise_accel_stddev_z', args.imu_noise_std)
        imu_bp.set_attribute('noise_gyro_stddev_x', args.imu_noise_std)
        imu_bp.set_attribute('noise_gyro_stddev_y', args.imu_noise_std)
        imu_bp.set_attribute('noise_gyro_stddev_z', args.imu_noise_std)
    if args.imu_bias:
        imu_bp.set_attribute('noise_gyro_bias_x', args.imu_bias_std)
        imu_bp.set_attribute('noise_gyro_bias_y', args.imu_bias_std)
        imu_bp.set_attribute('noise_gyro_bias_z', args.imu_bias_std)
...
def main(arg):
    ...
    try:
        ...
        # if not arg.basemap:
        #     #add offset to the start positoin to avoid exact same path
        #     x_offset = random.uniform(-0.2, 0.2) 
        #     y_offset = random.uniform(-0.2, 0.2) 
        #     route_points[0].location.x += x_offset
        #     route_points[0].location.y += y_offset
        ...
        #args.lanes shouldnt be greater that len(vehicle_locations)
        # if arg.lanes > len(vehicle_locations):
        #     print("args.lane cannot be greater than len(vehicle_locations)")
        #     return False
        ...
        lidar_car.listen(lambda data : lidar_car_callback(world, data))

        # #create car imu sensor
        imu_bp = generate_imu_bp(arg, blueprint_library)
        ...
        imu_sensor.listen(lambda imu: imu_callback(world, imu))
        ...
        while True:
            ...
            # Reading data from vehicle imu Queuq
            i_car= imu_car_sensorQueue.get(True,5)
            i_car[0] = current_frame
