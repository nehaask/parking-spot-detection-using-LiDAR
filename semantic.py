...
# # Generate continuous waypoints along the route to cover the entire path
# def generate_waypoints_along_route(start_transform, distance_interval=5.0):
#     current_waypoint = map.get_waypoint(start_transform.location)
#     waypoints = [current_waypoint]
#     while True:
#         next_waypoints = current_waypoint.next(distance_interval)
#         if not next_waypoints:
#             break
#         # Follow the first next waypoint (assuming straight path)
#         current_waypoint = next_waypoints[0]
#         waypoints.append(current_waypoint)
#         # Stop if we've reached the end of the route points
#         if len(waypoints) > 100:  # Prevent infinite loop
#             break
#     return waypoints
